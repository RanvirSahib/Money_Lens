...
import { FinancialChatbotDrawer } from './ai/FinancialChatbotDrawer';
...
function MoneyLensAppInner({ initialScreen = 'landing' }: MoneyLensAppProps) {
  ...
  return (
    <div className="bg-slate-50 text-slate-900 selection:bg-emerald-100 selection:text-emerald-900 font-sans antialiased min-h-screen flex flex-col relative bg-grid-subtle">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentScreen === 'profile' && (
              <ProfileScreen
                onProfileUpdated={(updated) => {
                  setIncome(updated.monthly_income);
                  setExpenses(updated.total_monthly_expenses);
                  setSavings(updated.current_savings);
                  setHealthScore(updated.health_score);
                  updateUserParams(updated.monthly_income, updated.total_monthly_expenses, updated.current_savings);
                  showToast('Financial Profile updated and engine recalibrated.');
                }}
              />
            )}
            ...
            {currentScreen === 'statements' && (
              <StatementsScreen
                onNavigateToSpending={() => setCurrentScreen('spending')}
      ...
      {/* Financial Chatbot Drawer */}
      <FinancialChatbotDrawer
        isOpen={isChatOpen}
        onClose={() => setIsChatOpen(false)}
        onProfileUpdated={() => {
          showToast('Profile updated from conversational assistant.');
        }}
        onGoalCreated={() => {
          showToast('Goal created from conversational assistant.');
        }}
      />
